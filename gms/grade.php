...
<html lang="en">
	...
	<body>	<?php include('include/header.php'); ?>
		...
		<?php
			...
			$batch_set = $connection->query($sql);
			if (!$batch_set) { die("Could not read from database.". mysqli_error($connection)); }
		?>
		<?php
			...
			if (isset($_GET['initial_Submit']))
			{
				...
				$subject_set = mysqli_query($connection,$subject_sql);
				if (!$subject_set) {
					die ("Could not read from database to read courses".mysqli_error($connection));
				}
			}
			...
			elseif (isset($_GET['action']) && isset($_GET['code']) && $_GET['action'] == "addgrade") {
				...
				// $student_set = mysqli_query($student_sql);
				// if (!$student_set) {
				...
				$student_set = mysqli_query($connection,$student_sql);
				if (!$student_set) {
			...
			elseif (isset($_POST['grade_submit'])) {
				...
				for ($i=1; $i <= $num; $i++) {
					...
					if (!$result_set) {
						die("Couldn't update student records.".mysqli_error($connection));
					}
				}
							...
							<div class="controls">
								<select name="batch">
									<?php
										...
										$temp = "";
										while ($batches = mysqli_fetch_assoc($batch_set))
										{
											$selected = "";
								...
								if (mysqli_num_rows($subject_set) != 0)
								{
									...
									<table class="table table-hover">
										...
										<tbody>
											<?php
												$i=1;
												while ($course = mysqli_fetch_array($subject_set))
												{
													echo "<tr>";
													echo "<td>{$i}</td>";
