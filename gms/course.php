							...
							<ul class="nav nav-list">
								<?php $active=" class=\"active\" "; ?>
							 	<li class="nav-header">Courses</li>
							  	...
							  	<li <?php if ((isset($_GET['action']) and $_GET['action'] == "assign_instructor") || (isset($_GET['assign_instructor']))) {
							  		echo $active;
							  	}?> ><a href="course.php?action=assign_instructor">Assign/change instructor</a></li>
					...
					if(isset($_GET['action']))
					{
						...
						if($act=='add')
						{
							...
							<div class='well container-fluid span7'	>
								...
								<form class="form-signin" action="course.php" onsubmit='return confrm()' name="add_sub" method="post">
									...
									<input type="text" class="input-block-level-large" placeholder="Credit" name="credit" pattern="^[0-9]{1,2}$" title="Please enter a valid credit course" required><br>
						...
						else if($act=='assign_instructor')
						{
							if(isset($_POST['change']))
							{
								...
								echo $class_id.$course_code.$new_teacher;
							...
							{ 
								...
								while($counter!=0)
								{
									...
									if($aa!=$bb)
									{
										echo"<OPTION value=".$tadaa['class_id'].">".convert($tadaa['class_id'])."</option>";
										$bb=$aa;
									}
									$counter=$counter-1;
								}
...
<?php
	...
	else if(isset($_POST['update']))
	{
		...
		//now add to database
		$query = mysqli_query($connection,"UPDATE courses SET course_name='$course_name', credit='$credit', description='$description' WHERE course_code='$course_code'") or die(mysqli_error($connection));
		if($query)
		{
