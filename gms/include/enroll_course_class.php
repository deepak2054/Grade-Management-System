...
<?php
	...
	if(isset($_POST['enroll']))
	{
		...
		//$temp_batch=substr($_POST['batch'],-2); //this is for the taking the last two strings
		...
		$class_id=$temp."s".$temp_batch;//this is for making the class code. this is done by manuplating the class code by the combination of the
		//batch,semester
		echo $class_id;
		...
		if(isset($_POST['set']) && isset($_POST['instructor']))
		{
			...
			foreach($_POST['set'] as $set)
			{
				...
				$test_set = mysqli_query($connection,$test);
				if (!$test_set) {
					$enroll_course_success = 0;
				}
				elseif (mysqli_num_rows($test_set) != 0) {
					$enroll_course_success = -1;
				}
				else {
