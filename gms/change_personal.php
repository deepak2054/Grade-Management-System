...
if (isset($_POST['submit'])) {
    ...
    $user_set = $connection->query($sql) or die("Couldn't connect to database");
    if (mysqli_num_rows($user_set) == 0) {
        ...
        $sql_set = mysqli_query($connection, $sql) or die("Couldn't connect to database");
        $message = "Record successfullly updated";
    }
}
$sql            = "SELECT * FROM user_detail WHERE user_name='" . logged_in_username() . "'";
$sql_set        = $connection->query($sql) or die("oops");
$teacher_record = mysqli_fetch_assoc($sql_set);
