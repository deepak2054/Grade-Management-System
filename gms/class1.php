		...
		<div class="row-fluid">
			...
			<div class="span9">
				<?php
					...
					{
						$edit_button=0;//<To check either the enroll button is pressed our not in the enroll student section
						...
						else if($act=='enroll')
						{
							...
							if(isset($_POST['enroll']))
							{
								...
								$s="DELETE FROM `gms`.`student_grade` WHERE reg_no='".$reg_id."' AND class_id='".$class_id."'";//this must be deleted first inorder to remove the subjects selected by the student first and then the new updated data is set to the table
							...
							else if(isset($_POST['edit']))
							{
								...
								echo "<h1>".$reg_no."</h1>";
								$qu=mysqli_query($connection,"SELECT * FROM student WHERE reg_no='$reg_no'");
									...
									<form action='class1.php?action=enroll' method='POST'>
										<table class="table table-hover">
											...
											<tbody>
												...
												$query=mysqli_query($connection,"SELECT * FROM class WHERE class_id='$class_id'");
											    ...
											    //  now display the information from the db
													...
													$flag=0;
													$query1=mysqli_query($connection,"SELECT * FROM student_grade WHERE reg_no='$reg_no'");
														$flag_temp=0;
							...
							if(!(isset($_POST['enroll']))|| ($button_press_checkor==1))
							{
								?>
					...
					else if(isset($_GET['course_action']))
					{
						...
						$q=mysqli_query($connection,$sq) or die(mysqli_error($connection));
